...
const HomePage1 = ({ navigateToEnroll, currentUserId }) => {
  ...
  const [allCourses, setAllCourses] = useState([]); // Define allCourses state
      ...
      try {
        ...
        const coursesData = querySnapshot.docs.map((doc) => {
          ...
          return {
            ...
            ...data,
            imageUrl,
            enrolledStudentsCount: data.enrolledStudents ? data.enrolledStudents.length : 0
          };
        });
        ...
        // Set allCourses state with fetched data
        ...
        {(showFilteredCourses ? filteredCourses : featuredCourses).map((course) => (
          <div className="col-md-4 mb-4" key={course.id}>
            ...
            {course.ratings && (
              <div>
                <ul>
                  {Object.entries(course.ratings).map(([userId, rating]) => (
                    <li key={userId}>
                      <h6>Ratings: {rating} stars</h6>
                    </li>
                  ))}
                </ul>
              </div>
